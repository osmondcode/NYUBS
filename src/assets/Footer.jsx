import Logo from "./IMAGES/LOGO.jpeg";

const Footer = () => {
  return (
    <section className="w-full min-h-[50vh] bg-[var(--dgreen)] text-[var(--bg)] px-[8%] pt-[30px] flex items-start justify-around">
      <div className="w-fit flex flex-col items-start justify-start gap-7">
        <span className="w-fit flex items-center gap-2">
          <div className="w-[27px] h-[27px] rounded-full">
            <img
              src={Logo}
              alt="LOGO"
              className="w-full h-full rounded-full flex items-center justify-center object-cover"
            />
          </div>
          <h2 className="font-semibold text-[17px] poppins">NYUBS</h2>
        </span>

        <div className="w-fit flex flex-col gap-2 text-[12px] text-[var(--gray)]">
          <p className="raleway underline underline-offset-4 font-light">
            Home
          </p>
          <p className="raleway underline underline-offset-4 font-light">
            About
          </p>
          <p className="raleway underline underline-offset-4 font-light">
            Services
          </p>
          <p className="raleway underline underline-offset-4 font-light">RSA</p>
          <p className="raleway underline underline-offset-4 font-light">NYR</p>
          <p className="raleway underline underline-offset-4 font-light">
            Gallery
          </p>
        </div>
      </div>

      <div className="w-fit flex flex-col items-start justify-start gap-2">
        <span className="w-fit flex items-center gap-2">
          <h2 className="font-semibold text-[14px] poppins">Other Links</h2>
        </span>
      </div>

      <div className="w-fit flex flex-col items-start justify-start gap-2">
        <span className="w-fit flex items-center gap-2">
          <h2 className="font-semibold text-[14px] poppins">Follow Us</h2>
        </span>
      </div>

      <div className="w-fit flex flex-col items-start justify-start gap-2">
        <span className="w-fit flex items-center gap-2">
          <h2 className="font-semibold text-[14px] poppins">Newsletter</h2>
        </span>
      </div>
    </section>
  );
};
export default Footer;
